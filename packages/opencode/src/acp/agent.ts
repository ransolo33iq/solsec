...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@solsec-ai/sdk/v2"
...
async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
  ...
  const opencodeProvider = providers.find((p) => p.id === "solsec")
  if (opencodeProvider) {
