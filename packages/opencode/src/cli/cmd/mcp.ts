...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in solsec.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .solsec/ subdirectory too)
  const candidates = [path.join(baseDir, "solsec.json"), path.join(baseDir, "solsec.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".solsec", "solsec.json"), path.join(baseDir, ".solsec", "solsec.jsonc"))
  }
