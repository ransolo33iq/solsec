...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall solsec and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      ...
      brew: "brew uninstall solsec",
      choco: "choco uninstall solsec",
      scoop: "scoop uninstall solsec",
    }
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".solsec/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".solsec/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".solsec/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".solsec"))
    ) {
      continue
    }
