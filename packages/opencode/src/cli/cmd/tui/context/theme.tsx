...
import solsec from "./theme/solsec.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@solsec-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "solsec")
        draft.active = typeof active === "string" ? active : "solsec"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.solsec, store.mode)
    })
