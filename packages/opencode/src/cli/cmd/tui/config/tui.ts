...
import type { DeepMutable } from "@solsec-ai/core/schema"
import type { TuiAttentionSoundName } from "@solsec-ai/plugin/tui"
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old solsec.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.solsec`
  ...
  const dirs = unique(directories).filter((dir) => dir.endsWith(".solsec") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".solsec") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
