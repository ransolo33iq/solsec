...
import { Flock } from "@solsec-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".solsec"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".solsec", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".solsec", "tui.json"),
  }
}
