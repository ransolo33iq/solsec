...
import { NamedError } from "@solsec-ai/core/util/error"
import { Flag } from "@solsec-ai/core/flag/flag"
import { Auth } from "../auth"
import { Env } from "../env"
import { applyEdits, modify } from "jsonc-parser"
import { InstallationLocal, InstallationVersion } from "@solsec-ai/core/installation/version"
...
import { NonNegativeInt, PositiveInt, type DeepMutable } from "@solsec-ai/core/schema"
...
// Uses the shared `DeepMutable` from `@solsec-ai/core/schema`. See the definition
...
function globalConfigFile() {
  const candidates = ["solsec.jsonc", "solsec.json", "config.json"].map((file) =>
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        ...
        const updated = text.replace(/^\s*\{/, '{\n  "$schema": "https://solsec.ai/config.json",')
    ...
    const loadGlobal = Effect.fnUntraced(function* () {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "solsec.json")))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "solsec.jsonc")))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("solsec", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".solsec") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["solsec.json", "solsec.jsonc"]) {
          ...
          // Auto-discovered plugins under `.solsec/plugin(s)` are already local files, so ConfigPlugin.load
