...
- `ai-sdk.ts` converts AI SDK `fullStream` parts into `@solsec-ai/llm` `LLMEvent`s. This is the default runtime path.
- `native-request.ts` converts opencode's normalized session input into a native `@solsec-ai/llm` `LLMRequest`. It does not execute requests.
...
`native-runtime.ts` evaluates the gate and either bridges into `@solsec-ai/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.
