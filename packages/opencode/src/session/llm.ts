...
import type { LLMEvent } from "@solsec-ai/llm"
import { LLMClient, RequestExecutor } from "@solsec-ai/llm/route"
import type { LLMClientService } from "@solsec-ai/llm/route"
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const run = Effect.fn("LLM.run")(function* (input: StreamRequest) {
      ...
      const opencodeProjectID = input.model.providerID.startsWith("solsec")
        ? (yield* InstanceState.context).project.id
        : undefined

      const requestHeaders = {
        ...(input.model.providerID.startsWith("solsec")
          ? {
              ...(opencodeProjectID ? { "x-opencode-project": opencodeProjectID } : {}),
              "x-opencode-session": input.sessionID,
              "x-opencode-request": input.user.id,
              "x-opencode-client": flags.client,
              "User-Agent": `solsec/${InstallationVersion}`,
            }
          : {
