...
import { EffectFlock } from "@solsec-ai/core/util/effect-flock"
...
import { CrossSpawnSpawner } from "@solsec-ai/core/cross-spawn-spawner"
...
async function writeManagedSettings(settings: object, filename = "solsec.json") {
...
async function writeConfig(dir: string, config: object, name = "solsec.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "solsec.json") =>
...
test("creates global jsonc config with schema when no global configs exist", async () => {
  ...
  try {
    ...
    const content = await Filesystem.readText(path.join(tmp.path, "solsec.jsonc"))
...
test("does not create global config when OPENCODE_CONFIG_DIR is set", async () => {
  ...
  try {
    ...
    expect(await Filesystem.exists(path.join(tmp.path, "solsec.jsonc"))).toBe(false)
  } finally {
...
test("updates global config and omits empty shell key in json", async () => {
  ...
  try {
    ...
    const writtenConfig = await Filesystem.readJson<{ shell?: string }>(path.join(tmp.path, "solsec.json"))
...
test("updates global config and omits empty shell key in jsonc", async () => {
  ...
  try {
    ...
    const file = path.join(tmp.path, "solsec.jsonc")
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.promise(() =>
      Filesystem.write(
        path.join(test.directory, "solsec.jsonc"),
        `{
        // This is a comment
        "$schema": "https://opencode.ai/config.json",
        "model": "test/model",
...
it.instance("preserves env variables when adding $schema to config", () =>
  withProcessEnv(
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Filesystem.write(
          path.join(test.directory, "solsec.json"),
          JSON.stringify({
      ...
      const content = yield* Effect.promise(() => Filesystem.readText(path.join(test.directory, "solsec.json")))
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* Effect.promise(() => Filesystem.write(path.join(test.directory, "solsec.json"), "{ invalid json }"))
    const exit = yield* Config.Service.use((svc) => svc.get()).pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .opencode directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".solsec", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".solsec", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .solsec/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".solsec", "agents", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .solsec/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".solsec", "command", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .solsec/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* mkdirEffect(path.join(test.directory, ".solsec", "commands", "nested"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const opencodeDir = path.join(projectDir, ".solsec")
      await fs.mkdir(opencodeDir, { recursive: true })

      // Global config with plugins
      await Filesystem.write(
        path.join(dir, "solsec.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
      ...
      await Filesystem.write(
        path.join(opencodeDir, "solsec.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".solsec", "agent"))
    yield* writeTextEffect(
      path.join(test.directory, ".solsec", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
test("keeps plugin origins aligned with merged plugin list", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      const local = path.join(project, ".solsec")
...
it.instance("local .opencode config can override MCP from project config", () =>
  Effect.gen(function* () {
    ...
    yield* mkdirEffect(path.join(test.directory, ".solsec"))
    yield* writeConfigEffect(
      path.join(test.directory, ".solsec"),
      {
        $schema: "https://opencode.ai/config.json",
        mcp: {
          docs: {
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    await using tmp = await tmpdir()
    const file = path.join(tmp.path, "solsec.json")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-opencode@2.4.3", "file:///project/.solsec/plugin/oh-my-opencode.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.solsec/plugin/demo.ts", "file:///project/.solsec/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.solsec/plugin/demo.ts"])
  })
...
describe("OPENCODE_DISABLE_PROJECT_CONFIG", () => {
  ...
  test("skips project .solsec/ directories when flag is set", async () => {
    const originalEnv = process.env["OPENCODE_DISABLE_PROJECT_CONFIG"]
    process.env["OPENCODE_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          ...
          const opencodeDir = path.join(dir, ".solsec", "command")
          await fs.mkdir(opencodeDir, { recursive: true })
