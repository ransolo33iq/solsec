...
const globalConfigFiles = ["solsec.json", "solsec.jsonc", "tui.json", "tui.jsonc"].map((file) =>
...
it.instance("keeps server and tui plugin merge semantics aligned", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const local = path.join(test.directory, ".solsec")
      yield* fs.makeDirectory(local, { recursive: true })

      yield* fs.writeJson(path.join(Global.Path.config, "solsec.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), {
        plugin: [["shared-plugin@1.0.0", { source: "global" }], "global-only@1.0.0"],
      })
      yield* fs.writeJson(path.join(local, "solsec.json"), {
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(test.directory, ".solsec", "tui.json"),
...
it.instance("migrates tui-specific keys from opencode.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      const source = path.join(test.directory, "solsec.json")
...
it.instance("migrates project legacy tui keys even when global tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(Global.Path.config, "tui.json"), { theme: "global" })
      yield* fs.writeJson(path.join(test.directory, "solsec.json"), {
      ...
      const server = JSON.parse(yield* fs.readFileString(path.join(test.directory, "solsec.json")))
      expect(server.theme).toBeUndefined()
      expect(server.tui).toBeUndefined()
    }),
  ),
...
it.instance("skips migration when opencode.jsonc is syntactically invalid", () =>
  ...
  "username": "still-broken"
      ...
      const source = yield* fs.readFileString(path.join(test.directory, "solsec.jsonc"))
...
it.instance("skips migration when tui.json already exists", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
      yield* fs.writeJson(path.join(test.directory, "solsec.json"), { theme: "legacy" })
...
it.instance("migrates legacy tui keys across multiple opencode.json levels", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeJson(path.join(test.directory, "solsec.json"), { theme: "root-theme" })
      yield* fs.writeJson(path.join(nested, "solsec.json"), { theme: "nested-theme" })
...
it.instance("does not derive tui path from OPENCODE_CONFIG", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeJson(path.join(customDir, "solsec.json"), { model: "test/model" })
      yield* fs.writeJson(path.join(customDir, "tui.json"), { theme: "should-not-load" })

      yield* withEnv(
        "OPENCODE_CONFIG",
        path.join(customDir, "solsec.json"),
        Effect.gen(function* () {
          const config = yield* getTuiConfig(test.directory)
          expect(config.theme).toBeUndefined()
        }),
...
it.instance("loads .solsec/tui.json", () =>
  withCleanState(
    Effect.gen(function* () {
      const fs = yield* AppFileSystem.Service
      const test = yield* TestInstance
...
it.instance("silently skips malformed tui.json - load failures degrade to {}", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(path.join(test.directory, ".solsec", "tui.json"), JSON.stringify({ theme: "fallback" }))
