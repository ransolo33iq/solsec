...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "solsec.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "solsec.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "solsec.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "solsec.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["solsec.json", "solsec.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "solsec.json"),
        path.join(tmp.path, "solsec.jsonc"),
        path.join(project, "solsec.json"),
        path.join(project, "solsec.jsonc"),
      ])
    })
  })
