...
- Author package entrypoint is `@solsec-ai/plugin/tui`.
...
- Import types from `@solsec-ai/plugin/tui`.
- `@solsec-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@solsec-ai/plugin/tui"
...
When `plugin` entries exist in a writable `.opencode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@solsec-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@solsec-ai/plugin/tui`.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@solsec-ai/plugin/tui`.
...
- Local plugins persist installed themes under the local `.solsec/themes` area near the plugin config source.
...
- Local smoke plugin: `.solsec/plugins/tui-smoke.tsx`
- Local vim plugin: `.solsec/plugins/tui-vim.tsx`
- Local smoke config: `.solsec/tui.json`
- Local smoke theme: `.solsec/plugins/smoke-theme.json`
